...
export default function ResumePreview({ content, profile, showSummary = true }: ResumePreviewProps) {
          ...
          <section key="education" className="mb-5">
            ...
            {content.education.map((edu, i) => (
              <div key={edu.id || i} className="mb-2 last:mb-0">
                ...
                <p className="text-sm text-gray-600">{edu.school}{edu.gpa ? ` · GPA: ${edu.gpa}` : ''}</p>
                {edu.description && (
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-8 max-w-[800px] mx-auto" id="resume-preview">
      {/* Header */}
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{profile.full_name}</h1>
        <div className="flex items-center justify-center gap-2 flex-wrap mt-1 text-sm text-gray-600">
