...
import { addExperience, updateExperience, deleteExperience } from '@/services/actions'
...
export default function ExperienceList({ experiences: initialExperiences }: { experiences: Experience[] }) {
  ...
  const onSubmit = async (data: ExperienceFormData) => {
    try {
      if (editingId) {
        ...
        const added = await addExperience({ ...data, sort_order: experiences.length })
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-base text-muted-foreground">{experiences.length} entries</p>
        </div>
        {!showForm && (
          <Button onClick={() => { setShowForm(true); setEditingId(null); reset({ company: '', title: '', start_date: '', end_date: '', is_current: false, description: '' }); }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  {errors.company && <p className="text-base text-destructive">{errors.company.message}</p>}
                </div>
                <div className="space-y-2">
      ...
      {experiences.length === 0 && !showForm && (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-base text-muted-foreground/60 mt-1">Click &quot;Add Experience&quot; to get started.</p>
          </CardContent>
        </Card>
      )}
